...
[1편](/blog/db-partitioning-performance)에서 인덱스를 추가해 1시간 → 51초로 줄였고, [2편](/blog/actionlog-optimization-2)에서는 파티셔닝을 적용하기 전에 쿼리를 정비해야 한다는 걸 알게 됐다.

이제 23개 쿼리 전부 인덱스를 제대로 활용하고, 파티션 프루닝도 작동할 수 있는 상태다.
...
여기서 눈치 빠른 분은 한 가지 이상한 점을 발견했을 것이다.
...
"그러면 id의 유니크성이 깨지는 거 아닌가?"라고 걱정할 수 있다. 이론적으로는 다른 파티션에 같은 id가 들어갈 수 있다. 하지만 실제로는 `AUTO_INCREMENT`가 테이블 레벨에서 동작하기 때문에 같은 id가 생길 일이 없다.
...
"이론은 알겠는데, 실제로 효과가 있나?"라는 의문이 들 수 있다. 비슷한 규모에서 파티셔닝을 적용한 실제 사례들을 몇 가지 소개한다.
