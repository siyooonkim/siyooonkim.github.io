...
솔직히 좀 놀랐다. 인덱스 두 개 추가한 것뿐인데, 이 정도 차이가 나올 줄은 몰랐다. B-Tree라는 자료구조의 힘이 이런 거구나 싶었다.

그런데 51초도 마냥 빠르다고 할 수는 없었다. 인덱스가 범위를 잡아줬지만, 매칭된 수천만 건의 실제 데이터를 디스크에서 읽어와야 했고, `GROUP BY userId`로 전부 집계해야 결과가 나왔기 때문이다.

그리고 무엇보다, 다음 단계인 파티셔닝을 준비하면서 **예상 못한 문제**를 발견하게 된다.
