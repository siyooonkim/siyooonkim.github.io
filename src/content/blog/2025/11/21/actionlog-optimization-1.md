...
개발만 하다가 직접 쿼리문을 치면서 데이터를 뽑는게 재미(?)있다.
(SQL이 재미있다는게 아니라, 현재 작업에서 고통받고 있다는 뜻이다.)

이번에는 유저의 Action Log 기반의 테이블 기반에서 데이터를 추출해야했다. 

쿼리를 돌려놓은지 1시간이 지났는데도 추출되지 않았고, 뭔가 근본적인 문제가 있다는 신호로 느껴졌다. 

해당 ActionLog테이블을 보니, 이미 수십억 건이 쌓여있엇고 (무슨 이유에서인지) 날짜 기반 인덱스가 적용되어있지 않았다.

유저 액션로그 데이터가 CRM 스쿼드에서 활용도가 굉장히 높다는걸 알게 되었고, 
ActionLog 테이블을 **인덱스 최적화 → 쿼리 수정 → 파티셔닝** 3단계에 걸쳐 최적화 하는 작업을 진행해보았따. 
---
...
당최 이해가 안 됐다. 수십억 건짜리 로그 테이블에서 기간 조회가 가장 빈번한 패턴인데, 왜 `createDate`에 인덱스가 안 걸려있었는지. 테이블이 처음 만들어진 시점에는 데이터가 적어서 필요 없었을 수도 있고, 이후로 누구도 건드리지 않았을 수도 있다. 어쨌든 9개 인덱스 중 가장 필요한 하나가 빠져있었다.
...
여러 서비스에 흩어져 있는 ActionLog 관련 쿼리를 전부 찾아서, 어떤 인덱스가 실제로 사용되는지 매핑했다. 인덱스를 무작정 추가하기 전에 기존 인덱스가 제 역할을 하고 있는지부터 파악해야 했기 때문이다.

9개 인덱스 중 활발히 사용되는 건 6개, JOIN에서만 쓰이는 게 1개, 그리고 INSERT 시에만 갱신되고 SELECT에서는 아무도 안 쓰는 게 2개였다. 인덱스는 읽기를 빠르게 해주지만, **쓰기를 느리게 만든다**. 수십억 건이 INSERT되는 테이블에서 아무도 안 쓰는 인덱스가 매번 갱신되고 있었다. 이 2개는 제거 후보로 분류했다.
