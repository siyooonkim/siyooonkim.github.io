...
처음에는 대충 `if` 문으로 분기 처리를 할까 고민했지만<br>
...
### 핵심은 **추상화**다.
서울 API와 경기도 API는 응답 형태가 다르다. <br>
서울은 JSON을 사용하지만, 경기도는 XML을 사용하고 필드명도 `busRouteId`와 `routeId`처럼 서로 다르다. <br>

이런 차이를 호출하는 쪽에서 모두 처리하게 하면 코드가 복잡해진다.  <br>
그래서 이 차이를 내부에서 흡수하도록 설계했다. <br>
 
공통 인터페이스를 먼저 정의하고, 각 어댑터가 지역별 API 응답을 이 형태로 변환하도록 구성했다. <br>
...
```
function getAdapter(cityCode: string) {
  switch (cityCode) {
    case 'SEOUL':
      return seoulAdapter;
    case 'GYEONGGI':
      return gyeonggiAdapter;
  }
}
```
예를 들어, 경기도 API는 도착 시간을 분 단위로 제공하지만, 서울 API는 초 단위로 제공한다. <br>
이 차이는 어댑터 내부에서 공통 형태로 변환된다. <br>
 경기도는 값을 그대로 사용하고, 서울은 초 단위를 분 단위로 변환한다. <br>

결과적으로 호출하는 쪽에서는 서울과 경기를 구분할 필요 없이, 항상 동일한 형태의 데이터를 사용할 수 있다.  <br>

추상화를 통해 **호출하는 쪽에서는 이런 차이를 알 필요가 없도록 만든다.** <br>
...
이를 방지하기 위해 **애플리케이션 부트스트랩 시점에 DB에 저장된 `Reserved` 상태의 알림을 조회**하고, 타이머를 다시 등록하도록 했다.<br>
